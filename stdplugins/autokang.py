...
async def _(event):
    ...
    packshortname = f"AutoBorg_kang_{userid}"  # format: Uni_Borg_userid

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@AutoBorg_Kang.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"@AnimatedStickersGroup {userid}'s "
        packshortname = f"@AutoBorg_Animated"  # format: Uni_Borg_userid
    ...
    await event.edit("@AutoBorg is working be patient")
